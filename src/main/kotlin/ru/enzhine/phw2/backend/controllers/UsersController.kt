...
class UsersController {
    ...
    fun shopCartChange(@PathVariable id: Long?, @RequestParam act: String?, model: Model): String{
        if(id == null || act == null){
        ...
        return when(act){
            ...
            "rem" -> {
                if(orderDetails().getShopCart().plusDish(id, -1, dishOpt.get().quantity)){
                    model.addAttribute("remSuccess", true)
                }else{
                    model.addAttribute("remError", true)
                }
                shopCart(model)
            }
            "remall" -> {
    ...
    fun shopCartApply(model: Model): String{
        val userOrder = orderDetails()
    ...
    fun shopCart(model: Model): String{
        val userOrder = orderDetails()
        model.addAttribute("action", "shop-cart")
    ...
    @GetMapping("/order/{id}")
    fun orderExtra(@PathVariable id: Long?, model: Model): String {
        if(id == null){
            return "redirect:/user/"
        }
        val dishOpt = dishesRepo.findById(id)
    ...
    @GetMapping("/order")
    fun order(@RequestParam(required = false, defaultValue = "false", name = "delete") delete: Boolean, model: Model): String{
        model.addAttribute("refresh", true)
        val userOrder = orderDetails()
    ...
    @GetMapping("/proceed")
    fun proceed( model: Model): String{
        val userOrder = orderDetails()
        val unpaid = ordersService.getReadyOrders(userOrder.userID())
        if(unpaid.count() == 0) {
            return "redirect:/user/"
    ...
    @PostMapping("/proceed")
    fun proceed(@RequestBody body: MultiValueMap<String, String>,  model: Model): String{
        val userOrder = orderDetails()
        val unpaid = ordersService.getReadyOrders(userOrder.userID())
        if(unpaid.count() == 0) {
            return "redirect:/user/"
